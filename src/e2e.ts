...
export async function endToEnd(service: KvService, { type, subtype, path }: { type: 'deno' | 'userland', subtype?: 'in-memory' | 'napi' | 'remote' | 'sqlite', path: string }) {
    ...
    if (subtype === 'in-memory' || type === 'userland' && pathType === 'remote') {
        {
            ...
            await kv.set([ 'w1' ], 'v1');
            {
                ...
                if (result.value !== undefined && result.value.length === 2 && result.value.every(v => v.value === null && v.versionstamp === null)) result = await reader.read(); // skip initial empty state (racy)
    ...
    {
        const k1 = [ 'k1' ];
        await assertRejects(() => kv.atomic().delete(k1).set(k1, 0n).sum(k1, 2n).commit());
