...
export async function run({ command, cwd, args, stdinFn, stdoutFn, signal }: Opts): Promise<{ code: number, millis: number}> {
    ...
    const readStderr = async () => {
        for await (const line of p.stderr.pipeThrough(new TextDecoderStream()).pipeThrough(new TextLineStream())) {
            console.error(line);
    ...
    const readStdout = async () => {
        if (stdoutFn) {
            ...
            for await (const line of p.stdout.pipeThrough(new TextDecoderStream()).pipeThrough(new TextLineStream())) {
                console.log(line);
            }
        }
    }
